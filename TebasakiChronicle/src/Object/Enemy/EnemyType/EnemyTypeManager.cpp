...
void EnemyTypeManager::LoadEnemyData(const std::string& filePath)
{
	...
	//視界用コリジョン
	K_Math::Vector3 fovpos = { 56.f, 16.f, 0.f };
	...
	//攻撃動作遷移用コリジョン
	K_Math::Vector3 aapos = { 8.f, 0.f, 0.f };
	shapeSize[0] = 16.f; shapeSize[1] = 16.f; shapeSize[2] = 1.f;
	K_Physics::CollisionShape* aas = CC::CreateBoxShape(shapeSize[0], shapeSize[1], shapeSize[2]);
	//足元、頭上判定用コリジョン
	K_Math::Vector3 cfpos = { 0.f, 16.f, 0.f };
	K_Math::Vector3 chpos = { 0.f, -16.f, 0.f };
	...
	eType.emplace_back(new EnemyType("resource2", 1, 1, bs, rs, fovs, aas, cs, rpos, fovpos, aapos, cfpos, chpos));
